...
2. **서비스별/도구별 세부 권한 통제 (Fine-grained RBAC & Scopes)**
...
CREATE TABLE users (
    ...
    external_subject_id VARCHAR(255),            -- IdP 시스템의 고유 Sub ID
...
┌────────────────────────────────────────────────────────────────────────┐
│ Fastify Tools Gateway Pipeline                                         │
│                                                                        │
│ 1. [Auth Guard]:                                                       │
...
- **목표**: 키 기반 인증으로 게이트웨이 보안 잠금 및 권한별 도구 필터링
- **세부 작업**:
  1. `src/auth/key-verifier.ts`: SHA-256 해시 검증 및 Redis 캐싱 레이어 구현
  2. `src/auth/scope-guard.ts`: `tools/call` 실행 전 스코프 일치 검증 로직
  3. `src/proxy/tool-filter.ts`: `tools/list` 요청 시 유저 권한에 맞는 도구만 동적 반환
  4. 테스트 케이스 작성: 유효한 키, 만료된 키, 권한 없는 도구 호출 시 정상 차단 검증

### 🔹 Phase 3: 유저 & API Key 관리 REST API 엔드포인트
- **목표**: 사용자가 API 키를 발급/조회/삭제하고 자신의 권한을 확인할 수 있는 백엔드 API
- **세부 작업**:
  1. `POST /api/v1/auth/sso-callback`: 통합인증 로그인 후 JIT 유저 프로비저닝
...
3. **[API Key 생성 스크립트 작성]**: 시스템 관리자가 첫 번째 관리자용 API Key를 발급할 수 있는 CLI/스크립트를 구성합니다.
